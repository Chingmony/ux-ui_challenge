...
export default function UXChallengeApp() {
  const [view, setView] = useState<'home' | 'playing' | 'result' | 'finished'>('home');
  ...
  const getRank = () => {
    if (score === 20) return "UX PROPHET";
    if (score > 15) return "SENIOR DESIGNER";
    if (score > 10) return "PRODUCT LEAD";
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full py-8">
        ...
        <nav className="flex justify-between items-center mb-12">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-black tracking-tighter uppercase">Master Game</h1>
          </div>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <header className="mb-10">
                <h2 className="text-7xl md:text-9xl font-black tracking-tighter leading-[0.75] mb-4">
                  Design<br />
                  <span className="text-transparent" style={{ WebkitTextStroke: '2px black' }}>CHALLENGES</span>
                </h2>
                <p className="font-bold text-slate-500 uppercase tracking-widest text-sm italic">Select a unit to begin training</p>
              </header>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <span className="font-black text-xs opacity-30 italic">U_{i + 1}</span>
                        {isDone && <div className="h-4 w-4 bg-emerald-500 rounded-full border-2 border-black" />}
                      </div>
                      <div>
                        <h3 className="text-2xl font-black uppercase leading-[0.9] mb-3 tracking-tighter">
                          {c.title}
                        </h3>
            ...
            >
              <div className="bg-white border-[8px] border-black p-8 md:p-16 shadow-[30px_30px_0px_#4f46e5] relative">
                ...
                <div className="mt-8">
                  ...
                  <h2 className="text-5xl md:text-7xl font-black mb-16 leading-[0.9] tracking-tighter">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {challenges[currentLevel].options.map((opt) => (
                      ...
                      >
                        <span className={`text-3xl font-black uppercase group-hover:text-white leading-none mb-3 block ${opt.color}`}>
                          {opt.label}
                        </span>
          ...
          {view === 'result' && (
            ...
            >
              <div className={`p-12 border-[8px] bg-white shadow-[20px_20px_0px_black] ${selectedCorrect ? 'border-emerald-500' : 'border-rose-500'}`}>
                <h2 className={`text-9xl font-black mb-8 italic tracking-tighter ${selectedCorrect ? 'text-emerald-500' : 'text-rose-500'}`}>
                  {selectedCorrect ? 'GOT IT.' : 'NOPE.'}
                </h2>
                
                <div className="bg-black text-white p-8 mb-10">
                  <p className="text-indigo-400 font-black text-sm uppercase mb-4 tracking-[0.3em]">The UX Logic</p>
                  <p className="text-4xl font-black leading-[0.85] tracking-tight italic">"{challenges[currentLevel].lessonBody}"</p>
                </div>
                
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={handleNext} className="flex-1 py-6 bg-black text-white font-black uppercase text-3xl shadow-[8px_8px_0px_#4f46e5] hover:bg-indigo-600 transition-all">
                    {currentLevel === challenges.length - 1 ? 'Finish Game' : 'Next One →'}
                  </button>
                  <button onClick={() => setView('home')} className="px-8 py-6 border-[4px] border-black font-black uppercase text-sm hover:bg-slate-100 transition-all">
                    Dashboard
                  </button>
                </div>
              </div>
          ...
          {view === 'finished' && (
            ...
            >
              <div className="bg-white border-[10px] border-black p-16 shadow-[40px_40px_0px_#fbbf24]">
                <h2 className="text-2xl font-black uppercase mb-4">Evaluation Complete</h2>
                <div className="text-[12rem] font-black leading-none tracking-tighter mb-4">{score}/20</div>
                <div className="text-5xl font-black uppercase italic bg-black text-white inline-block px-8 py-2 mb-12">
                  {getRank()}
                </div>
                <button 
                  onClick={() => { setScore(0); setCompletedLevels([]); setView('home'); }}
